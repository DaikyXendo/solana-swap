...
import SolSwapInstructionService from './solswap-instruction.service';
...
export default class SolSwapService {
    ...
    async createLiquidityPool(payer: PublicKey, name: string) {
        ...
        const result = SolSwapInstructionService.initializePoolIxBase(
            name,
            payer,
    ...
    async swapToken(
        ...
        const result = await SolSwapInstructionService.swapToken(
            amount,
            senderAddress,
            poolAddress,
            poolAuthority,
